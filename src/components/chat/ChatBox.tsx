    ...
    <div className="flex flex-col w-full h-full bg-white rounded-3xl border border-slate-100 shadow-xl overflow-hidden relative">
      ...
      <header className="py-2.5 px-5 bg-gradient-to-r from-teal-500 to-teal-400 text-white flex items-center justify-between shadow-sm z-10 flex-shrink-0">
        ...
        <button
          ...
          className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-white text-teal-600 hover:bg-teal-50 active:scale-95 shadow-sm transition-all text-xs font-bold cursor-pointer select-none disabled:opacity-30 disabled:pointer-events-none"
      ...
      <div className="flex-1 overflow-hidden flex flex-col relative bg-slate-50/30">
        {messages.length === 0 ? (
          <div className="flex-1 overflow-y-auto px-6 pt-3 pb-6 md:pt-6 md:pb-8 flex flex-col items-center justify-start md:justify-center w-full scrollbar-none">
            <motion.div
              initial={{ scale: 0.8, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ duration: 0.5 }}
              className="mb-3 p-2.5 bg-teal-50 rounded-2xl border border-teal-100"
            >
            ...
            >
              ...
              <p className="text-slate-500 text-xs sm:text-sm max-w-md mx-auto leading-relaxed">
                Mình có thể giúp gì cho pet yêu của bạn hôm nay? Hãy chọn một chủ đề bên dưới hoặc gửi câu hỏi nhé!
              </p>
            </motion.div>
            ...
            >
              {suggestions.map((item, idx) => (
                <button
                  ...
                  className="flex flex-col text-left p-3 rounded-2xl bg-white border border-slate-200/80 hover:border-teal-300 hover:bg-teal-50/20 transition-all duration-300 group cursor-pointer shadow-sm relative overflow-hidden"
                >
                  <div className="flex items-center space-x-3 mb-1.5 relative z-10">
                    <div className="p-1.5 rounded-xl bg-slate-50 border border-slate-100 group-hover:bg-white group-hover:border-teal-100 transition-colors">
                      {item.icon}
                    </div>
