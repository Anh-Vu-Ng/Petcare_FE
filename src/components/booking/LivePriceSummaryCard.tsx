      ...
      <div className="bg-gradient-to-r from-teal-600 via-teal-500 to-teal-600 text-white p-4.5 sm:p-5 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between relative z-10">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="font-extrabold text-lg leading-tight">Phiếu Dịch Vụ Tạm Tính</h3>
              <p className="text-xs text-teal-100 font-medium">Cập nhật chi phí thời gian thực</p>
            </div>
          </div>

          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-white/20 text-white border border-white/30 backdrop-blur-xs">
      ...
      <div className="p-6 space-y-5 text-sm">
        ...
        {Array.isArray(estimate?.free_gifts) && estimate.free_gifts.length > 0 && (
          <div className="p-2.5 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-2xl">
            ...
            {estimate.free_gifts.map((gift, gIdx) => (
              <p key={gIdx} className="text-xs text-emerald-700 font-medium pl-4">
                • {gift}
              </p>
            ))}
          </div>
        ...
        <div className="pt-3 border-t border-dashed border-slate-200 space-y-1.5">
          {(Number(estimate?.discount_amount) || 0) > 0 && (
            <div className="flex items-center justify-between text-emerald-600 pb-1.5 border-b border-slate-100">
              <span>Giảm giá ưu đãi ({Number(estimate?.discount_percentage) || 0}%):</span>
          ...
          <div className="flex items-baseline justify-between pt-0.5">
            <span className="font-extrabold text-slate-900 text-base">TỔNG TIỀN DỰ KIẾN:</span>
            <div className="text-right">
              {isOnlyMedical ? (
                ...
                <span className="font-black text-teal-600 text-2xl tracking-tight">
        ...
        {/* Disclaimer Note */}
        {estimate.disclaimer && (
          <div className="p-2.5 bg-amber-50/80 border border-amber-200/70 rounded-2xl text-xs text-amber-800 leading-relaxed flex items-start gap-2">
        ...
        <div>
          ...
          <textarea
            ...
            {...register('notes')}
            className="w-full p-2.5 bg-slate-50/80 border border-slate-200 rounded-2xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-100 focus:bg-white transition-all resize-none"
          />
        </div>
        ...
        <button
          ...
          className="w-full py-3.5 px-4 rounded-2xl bg-gradient-to-r from-teal-500 via-teal-600 to-teal-500 hover:from-teal-600 hover:to-teal-600 text-white font-extrabold text-base shadow-lg shadow-teal-500/25 hover:shadow-teal-500/35 active:scale-98 transition-all duration-200 flex items-center justify-center space-x-2 cursor-pointer select-none disabled:opacity-50 disabled:pointer-events-none"
        >
          {isSubmitting ? (
            <>
        ...
        <div className="flex items-center justify-center space-x-1 text-xs text-slate-400 pt-1">
